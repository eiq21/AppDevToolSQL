from DataAccess.connection_mssql import conn_sql
...
def from_sql(sp):
    query_line = []
    str_cnx_sql = util.get_value_json(
        get_config.get_json('credentials_database'))
    sql_access = conn_sql(str_cnx_sql[0], str_cnx_sql[1], str_cnx_sql[2],
                          str_cnx_sql[3])
...
def from_sql_v2(sp):
    ...
    sql_access = conn_sql(str_cnx_sql[0], str_cnx_sql[1], str_cnx_sql[2],
                          str_cnx_sql[3])
    conn = sql_access.connect()
    query = "select replace(definition, 'CREATE PROCEDURE', 'ALTER PROCEDURE') from sys.sql_modules m join sys.procedures p on m.object_id = p.object_id where name = '{}'".format(
        sp)
    cursor = conn.cursor()
    cursor.execute(query)
    row = cursor.fetchone()
